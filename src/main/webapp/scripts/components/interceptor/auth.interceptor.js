...
angular.module('transandalus')
    .factory('authExpiredInterceptor', function ($rootScope, $q, $injector, localStorageService, API_URL) {
        return {
            responseError: function(response) {
                // If we have an unauthorized request we redirect to the login page
                // Don't do this check on the account API to avoid infinite loop
                if (response.status == 401 && response.data.path !== undefined && response.data.path.indexOf(API_URL + "/account") == -1){
                    ...
                    return $http.get(API_URL).finally(function() { return afterCSRFRenewed(response); });
                }
